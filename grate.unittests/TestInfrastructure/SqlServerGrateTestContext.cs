...
class SqlServerGrateTestContext : TestContextBase, IGrateTestContext, IDockerTestContext
{
    ...
    public string DockerCommand(string serverName, string adminPassword) =>
        $"run -d --name {serverName} -e ACCEPT_EULA=Y -e SA_PASSWORD={adminPassword} -e MSSQL_PID=Developer -e MSSQL_COLLATION=Danish_Norwegian_CS_AS -P mcr.microsoft.com/mssql/server:latest";
